...
export default function App() {
  ...
  const handleInputChange = (e) => {
    ...
    if (inputs.type !== "wall_hanging") {
      if (name === "isHandle") {
        ...
        value === "true"
          ? setDisableTopCoversSides(false)
          : setDisableTopCoversSides(true);
        return;
      }
    }
  ...
  const handleExportCSV = () => {
    ...
    csvContent +=
      "Component Name,Quantity,Length (mm),Width (mm),Thickness (mm),Notes\n";
    ...
    link.setAttribute(
      "download",
      `${inputs.projectName || `Untitled - ${new Date().toISOString().slice(0, 19).replace("T", " ")}`}.csv`,
    );
      ...
      <aside className="w-80 bg-white border-r border-slate-200 flex flex-col h-full shadow-sm z-10 relative">
        <div className="p-5 border-b border-slate-200 bg-slate-800 text-white">
          ...
          <p className="text-xs text-slate-400 mt-1">
            A Elique Interiors Product
          </p>
        </div>

        <div className="p-5 flex-1 overflow-y-auto space-y-6">
          <div>
            <label className="block text-m font-semibold text-blue-800 uppercase tracking-wider mb-3">
              Project Name
            </label>
            <input
              type="text"
              ...
              className="w-full text-sm border-slate-300 rounded-md shadow-sm bg-slate-50 p-2 border"
            />
          </div>

          {/* Storage Type */}
          <div>
            <label className="block text-xs text-center font-semibold text-slate-500 uppercase tracking-wider mb-2">
              --------------- Storage Type ---------------
            </label>
            <select
              name="type"
          ...
          <div className="p-4 bg-blue-50 rounded-lg border border-blue-100">
            <label className="block text-xs font-semibold text-blue-800 uppercase tracking-wider mb-3">
              Outer Dimensions (mm)
            </label>
              ...
              {["W", "H", "D"].map((dim) => (
                <div key={dim} className="flex items-center justify-between">
                  <label className="text-sm text-slate-700">
                    {dim === "W" ? "Width" : dim === "H" ? "Height" : "Depth"} (
                    {dim})
                  </label>
                  <input
                    type="number"
                    ...
                    onChange={handleInputChange}
                    placeholder="e.g. 600"
                    className="w-24 text-sm border-slate-300 rounded p-1 text-right shadow-inner border"
                  />
                </div>
              ))}
            </div>
          ...
          {/* Core Parameters */}
          <div>
            <label className="block text-xs text-center font-semibold text-slate-500 uppercase tracking-wider mb-3">
              --------- Core Parameters (mm) ---------
            </label>
            <div className="space-y-3">
              {[
                ...
                >
                  ...
                  <input
                    ...
                    onChange={handleInputChange}
                    className="w-20 text-sm border-slate-300 rounded p-1 text-right border"
                  />
                </div>
              ))}
            </div>
          ...
          {/* Construction Style */}
          <div>
            <label className="block text-center text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2">
              -------------- Construction --------------
            </label>
            ...
            <div className="mt-2 flex  items-center gap-5 justify-start align-middle">
              <label className="flex items-center space-x-2 text-sm">
                <input
                  ...
                  name="constStyle"
                  value="top_covers"
                  checked={inputs.constStyle === "top_covers"}
                  onChange={handleInputChange}
                  className="text-blue-600 focus:ring-blue-500"
                  disabled={disabletopCoversSides}
                />
                <span>Yes</span>
              </label>
              ...
              <label className="flex items-center space-x-2 text-sm">
                <input
                  ...
                  name="constStyle"
                  value="sides_cover"
                  checked={inputs.constStyle === "sides_cover"}
                  onChange={handleInputChange}
                  className="text-blue-600 focus:ring-blue-500"
                  disabled={disabletopCoversSides}
                />
            ...
            {inputs.constStyle === "top_covers" &&
              inputs.type === "low_height" && (
                <div className="flex items-center justify-between mt-4">
                  <label className="text-sm text-slate-700 mr-2">
                    Top thickness
                  </label>
                  <input
                    type="number"
          ...
          <div>
            ...
            <div className="space-y-3">
              ...
              <div>
                <select
                  name="doorStyle"
                  value={inputs.doorStyle}
                  onChange={handleInputChange}
                  className="w-full text-sm border-slate-300 rounded p-2 border"
                >
          ...
          <div className="pt-4 border-t border-slate-200">
            <button
              ...
              className="w-full py-3 bg-slate-800 text-white text-sm font-bold rounded-md hover:bg-slate-700 transition shadow-sm"
            >
              Generate Cut List
            </button>
          </div>
      ...
      <main className="flex-1 flex flex-col h-full bg-slate-50 overflow-hidden relative">
        <header className="bg-white border-b border-slate-200 p-4 flex justify-between items-center shadow-sm z-10">
          <div>
            ...
            {error && (
              <div className="text-sm font-semibold text-red-600 mt-1 bg-red-50 p-2 rounded border border-red-200">
                {error}
              </div>
            )}
          </div>
          <div className="space-x-3">
            <button
              ...
              className="px-4 py-2 bg-white border border-slate-300 text-slate-700 text-sm font-medium rounded-md hover:bg-slate-50 transition shadow-sm"
            >
              Export CSV
            </button>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-8">
              ...
              <table className="w-full text-left text-sm whitespace-nowrap">
                <thead className="bg-slate-100 text-slate-600 font-semibold uppercase tracking-wider text-xs">
                  <tr>
                    ...
                    {/* <th className="p-3 rounded-tr-lg">Manufacturing Notes</th> */}
                  </tr>
                </thead>
          ...
          {cutList.length > 0 && (
            <section className="bg-white p-6 rounded-xl shadow-sm border border-slate-200">
              <h3 className="text-lg font-bold text-slate-800 mb-4 border-b pb-2">
                Technical Panel Blueprints (Scale: Auto)
              </h3>
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8">
                {cutList.map((part, index) => {
                  ...
                  return (
                    <div
                      key={index}
                      className="border border-slate-200 rounded-lg p-4 bg-white flex flex-col items-center relative group hover:border-blue-400 transition"
                    >
                      ...
                      >
                        ...
                        {hasGroove && (
                          <rect
                            x={x + 6 * scale}
                            y={y + 6 * scale}
                            width={rectW - 12 * scale}
                            height={rectH - 12 * scale}
                            fill="none"
                            stroke="#64748b"
                            strokeWidth="1.5"
                            strokeDasharray="6,4"
                        ...
                        <line
                          ...
                          stroke="#475569"
                          strokeWidth="1"
                          markerStart={`url(#arrow-start-${index})`}
                          markerEnd={`url(#arrow-end-${index})`}
                        />
                        <line
                          x1={x}
                          y1={y}
                        ...
                        />
                        <g
                          transform={`translate(${x + rectW + 25}, ${y + rectH / 2})`}
                        >
                          <text
                            x="0"
                            y="0"
