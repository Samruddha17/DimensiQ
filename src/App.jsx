        ...
        <div className="bg-slate-50 text-slate-800 h-screen flex overflow-hidden font-sans">
            ...
            <aside className="w-80 bg-white border-r border-slate-200 flex flex-col h-full shadow-sm z-10 relative">
                ...
                <div className="p-5 flex-1 overflow-y-auto space-y-6">
                    ...
                    {/* Storage Type */}
                    <div>
                        <label className="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2">Storage Type</label>
                        ...
                        <div className="space-y-3">
                            {['W', 'H', 'D'].map(dim => (
                                <div key={dim} className="flex items-center justify-between">
                                    <label className="text-sm text-slate-700">{dim === 'W' ? 'Width' : dim === 'H' ? 'Height' : 'Depth'} ({dim})</label>
                    ...
                    {/* Core Parameters */}
                    <div>
                        <label className="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-3">Core Parameters (mm)</label>
                        <div className="space-y-3">
                            {[
                    ...
                    {/* Construction Style */}
                    <div>
                        <label className="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2">Construction</label>
                        <div className="space-y-2">
                            <p>Top Covers Sides?</p>
                            <label className="flex items-center space-x-2 text-sm">
                                <input type="radio" name="constStyle" value="top_covers" checked={inputs.constStyle === 'top_covers'} onChange={handleInputChange} className="text-blue-600 focus:ring-blue-500" />
                                <span>Yes</span>
                            </label>
                            ...
                            <label className="flex items-center space-x-2 text-sm">
                                <input type="radio" name="constStyle" value="sides_cover" checked={inputs.constStyle === 'sides_cover'} onChange={handleInputChange} className="text-blue-600 focus:ring-blue-500" />
                                <span>No</span>
                            </label>
                        </div>
                        <div className="space-y-2">
                    ...
                    {/* Components */}
                    <div>
                        <label className="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2">Internal & Doors</label>
                        <div className="space-y-3">
                            ...
                            <table className="w-full text-left text-sm whitespace-nowrap">
                                ...
                                <tbody className="divide-y divide-slate-100 text-slate-700">
                                    {cutList.length === 0 && !error && (
                                        <tr><td colSpan="6" className="p-6 text-center text-slate-400 italic">Enter parameters and click "Generate Cut List"</td></tr>
                                    )}
                                    {cutList.map((part, idx) => (
                                        <tr key={idx}>
                                            ...
                                            <td className="p-3 text-right font-mono">{part.thick}</td>
                                            <td className="p-3 text-xs text-slate-500">{part.notes}</td>
                                        </tr>
                                    ))}
                                </tbody>
                            </table>
                                    ...
                                    return (
                                        <div key={index} className="border border-slate-200 rounded-lg p-4 bg-white flex flex-col items-center relative group hover:border-blue-400 transition">
                                            ...
                                            <svg viewBox={`0 0 ${viewSize} ${viewSize}`} width="100%" height="auto" className="max-w-xs font-mono" xmlns="http://www.w3.org/2000/svg">
                                                ...
                                                {/* Groove Line */}
                                                {hasGroove && (
                                                    <rect x={x + (6 * scale)} y={y + (6 * scale)} width={rectW - (12 * scale)} height={rectH - (12 * scale)} fill="none" stroke="#64748b" strokeWidth="1.5" strokeDasharray="6,4" />
                                                )}
                                                ...
                                                <text x={x + rectW / 2} y={y - 28} textAnchor="middle" fill="#1e293b" fontSize="16" fontWeight="bold">{part.dim1.toFixed(1)} mm</text>
                                                ...
                                                <g transform={`translate(${x + rectW + 25}, ${y + rectH / 2})`}>
                                                    <text x="0" y="0" transform="rotate(270)" textAnchor="middle" fill="#1e293b" fontSize="16" fontWeight="bold">{part.dim2.toFixed(1)} mm</text>
                                                </g>

                                                {/* Center Note */}
                                                <text x={x + rectW / 2} y={y + rectH / 2} textAnchor="middle" fill="#64748b" fontSize="14">T={part.thick}mm</text>
                                            </svg>
